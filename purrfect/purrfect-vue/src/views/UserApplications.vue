    ...
    <div class="content user-app-view">
        <div v-if="allApplications.length > 0" class="wrapperUserApplications d-flex flex-column align-items-center">
            <table class="table table-borderless mb-3" v-for="app in allApplications" :key="app.applicationId">
                <tbody>
                    <tr>
                        <td class="first-row"><h3>{{ app.jobListing.title }}</h3></td>
                        <td class="first-row text-end">
                            <span class="badge rounded-pill ">{{app.jobListing.kindOfJob}}</span>&nbsp;&nbsp;
                            <span class="badge rounded-pill ">Hourly Rate: ${{ app.jobListing.hourRate}}</span>&nbsp;&nbsp;
                            <span class="badge rounded-pill ">Total Hours: {{ calculateTotalHours(app.jobListing) }}</span>&nbsp;&nbsp;
                            <span class="badge rounded-pill ">Total Pay: ${{ calculateTotalCost(app.jobListing) }}</span>&nbsp;&nbsp;
                            <span class="badge rounded-pill ">Posted {{app.jobListing.postingDate}}</span>
                        </td>
                    </tr>

                    <tr>
                        <td colspan="4"><img :src="'data:image/jpg;base64,' + app.jobListing.image" alt="Job Image" class="job-image"/></td>
                    </tr>
                    <tr>
                        <td colspan="4">
                        <span class="badge rounded-pill ">Start Date: {{app.jobListing.jobStartDate}}</span>&nbsp;&nbsp;&nbsp;
                        <span class="badge rounded-pill ">Finish Date: {{app.jobListing.jobFinishDate}}</span>&nbsp;&nbsp;&nbsp;
                        <span class="badge rounded-pill ">Start Time: {{app.jobListing.startTime}}</span>&nbsp;&nbsp;&nbsp;
                        <span class="badge rounded-pill ">Finish Time: {{app.jobListing.endTime}}</span>&nbsp;&nbsp;&nbsp;
                        <span :class="app.isCancelled ? ('badge rounded-pill text-bg-danger') : ('badge rounded-pill')">Status:{{app.isCancelled ? 'Cancelled' : app.status }}</span>&nbsp;&nbsp;&nbsp;
                        </td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td colspan="4" class="last-row text-end">
                            <button class="btn btn-dark me-2 btn-purrfect-primary" :id="app.applicationId"
